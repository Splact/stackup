...
const VERBOSE = config.log && config.log.saga && !!config.log.saga.verbose || false;
...
function getLogPrefix(type, effect) {
  ...
  const char = isError ? '✘' : '✓';
  const winnerInd = effect && effect.winner ? char : '';
